...
from logger import setup_logger, get_monitor_logger
from monitor import MonitorEngine
from market import get_fetcher
...
async def test_connection():
    ...
    print("\n2. 测试AI服务...")
    ai = get_ai_service()
    comment = ai.generate_comment("BTC", 60000.0, 2.5, "minor", "嘲讽或戏谑")
    if comment:
        print(f"   ✅ AI服务正常")
    ...
    test_sent = notifier.telegram.send_message(
        "🧪 测试消息 - 加密货币监控系统已启动",
...
async def add_symbol(symbol: str):
    ...
    engine.add_symbol(symbol.upper())
...
async def remove_symbol(symbol: str):
    ...
    engine = MonitorEngine()
    await engine.initialize()
    engine.remove_symbol(symbol.upper())
